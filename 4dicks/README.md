...
- Collecting two Scatters awards 10 Free Spins; four award 20. An award popup explains the prize before the player continues into the bonus game.
- Free Spins use `src/assets/reels/bonus.png` while preserving the completed base-game state for the return screen. The source art has seven rows; canvas rendering reuses its cell artwork to create eight aligned rows and columns, with multiplier text centred using its visible glyph bounds. Each spin fills 1–64 cells. A three-second sonar scan selects a 90° angle, then stays visible for three seconds while its selected sector blinks. There is no separate quadrant overlay on the board. An X starts at ten times its final size and shrinks into its entry cell over three seconds. One thick line per side of the angle draws over the following three seconds before collection. If either angle ray points immediately outside the map, only the X cell is collected. The running total multiplies the settled main-game win. After all spins, a result popup reports the full amount.
- When the upcoming deck is drained and the reels contain no remaining diagonal match, Gamble Feature pauses settlement. The player can Collect the current win or choose Gamble. Gamble opens Hearts and Spades choices, then closes the choice popup for a fixed seven-second centre-canvas shuffle. The first five seconds shuffle quickly; the final two slow down before landing on a 50/50 Hearts or Spades result. A correct pick doubles the win; an incorrect pick settles at 0. The result holds briefly before the bonus and final settlement.
- Session totals remain in memory until reload. RTP is settled total winnings divided by total bets (spins × 1.00), displayed as a percentage. The statistics bar also records Free Spins Hits. Each spin counts when it starts; its winnings are added when it settles. Click/tap the canvas or Deal button to play; Space/Enter also work when the canvas is focused. Input is locked while a round is running.

...
import type { GameState, GridPoint, LaunchPoint, Player, RaySimulation, StaticBoard } from '../game/types.js';
import type { AnimationFrame } from './RayAnimator.js';

export const playerColors: Record<Player, string> = { X: '#27705b', O: '#b07817' };
interface View {
...
export class CanvasRenderer {
  ...
  draw(state: GameState, view: View) {
    ...
    ctx.fillStyle = '#f6f5ef'; ctx.fillRect(0, 0, this.width, this.height);
    for (const cell of this.board.cells) { const p = this.pixel(cell); ctx.fillStyle = '#fffdf7'; ctx.fillRect(p.x, p.y, s, s); }
    ctx.strokeStyle = '#e4e5dc'; ctx.lineWidth = 1; ctx.beginPath();
    ...
    for (const cell of state.cells) {
      ...
      if (cell.owner) { ctx.fillStyle = cell.owner === 'X' ? '#e0ece3' : '#f7e9c8'; ctx.fillRect(p.x + 2, p.y + 2, s - 4, s - 4); }
      if (cell.blocked) {
        ctx.fillStyle = '#343e38'; ctx.fillRect(p.x, p.y, s, s);
        ctx.save(); ctx.beginPath(); ctx.rect(p.x + 3, p.y + 3, s - 6, s - 6); ctx.clip();
        ctx.strokeStyle = '#526056'; ctx.lineWidth = 1;
        for (let n = -s; n < s * 2; n += 9) { ctx.beginPath(); ctx.moveTo(p.x + n, p.y); ctx.lineTo(p.x + n + s, p.y + s); ctx.stroke(); }
        ctx.restore();
      }
    }
    for (const e of this.board.staticEdges) this.line([{ x: e.x1, y: e.y1 }, { x: e.x2, y: e.y2 }], '#303c34', wallWidth);
    for (const trace of state.trajectories) for (const e of trace.edges) this.line([{ x: e.x1, y: e.y1 }, { x: e.x2, y: e.y2 }], playerColors[trace.player], traceWidth);
    for (const cell of state.cells) {
      ...
      ctx.strokeStyle = playerColors[cell.owner]; ctx.lineWidth = Math.max(1, s * 0.043); ctx.beginPath();
    ...
    if (view.active) {
      ...
      this.line(view.active.points, color, traceWidth + 1); this.circle(this.pixel(view.active.head), dotRadius + 0.5, '#fffdf7', color, traceWidth); ctx.restore();
    }
    if (view.preview?.termination === 'flat-wall' && state.phase === 'ready') {
      ...
      this.circle(this.pixel(view.preview.points[view.preview.points.length - 1]), dotRadius, '#fffdf7', color, Math.min(2, traceWidth));
    }
    if (state.phase !== 'finished') for (const launch of this.board.launchPoints) {
      ...
      this.circle(p, selected ? dotRadius * 1.5 : dotRadius, selected ? color : '#fffdf7', state.phase === 'animating' ? '#9ba69d' : color, Math.min(2, traceWidth));
      if (selected) {
    ...
    if (view.coordinates) {
      ctx.fillStyle = '#707c70'; ctx.font = `${s < 20 ? 8 : 10}px ui-monospace, monospace`; ctx.textAlign = 'center'; ctx.textBaseline = 'middle';
